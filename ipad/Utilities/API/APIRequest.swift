...
class APIRequest {
    ...
    private static func get(endpoint: URL, completion: @escaping (_ response: JSON?) -> Void) {
        ...
        // Make the call
        let _ = Alamofire.request(endpoint, method: .get, headers: headers()).responseData { (response) in
            completed = true
            if timedOut {return}
...
//        debugPrint(req)
    ...
    static func post(endpoint: URL, params: [String: Any], completion: @escaping (_ response: JSON?) -> Void) {
        ...
        // Request
        let _ = Alamofire.request(endpoint, method: .post, parameters: params, encoding: JSONEncoding.default, headers: headers()).responseJSON { response in
            completed = true
            if timedOut {return}
            guard response.result.description == "SUCCESS", let value = response.result.value else {
                return completion(nil)
